...
- 削除したタスクの復元用コピーは、削除した端末のIndexedDBだけに30日間・最大100件保存する。クラウドへは本文を再送せず、復元時は墓石との衝突を避けるため新しいIDのタスクとして作成する
...
- 通信成功なのに同じ変更が3回続けて受領確認されない場合は、クラウドと厳密一致なら反映済みとして整理し、不一致ならクラウドを上書きせず競合候補として1回だけ再構成する
- 再構成後も3回受領されなければ自動送信を停止し、タスクと送信記録を端末内に保持したまま設定画面へ再確認ボタンを表示する
- 自動復旧前の送信記録は端末内に最大50件保管し、本文を外部ログへ出さずに復旧件数と匿名タグだけを記録する
